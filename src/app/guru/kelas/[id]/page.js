              ...
              <tbody>
                {kelas.siswa.length === 0 ? (
                  ...
                  <tr key={siswa.nisn} style={{ backgroundColor: sIdx % 2 === 0 ? "var(--bg-secondary)" : "var(--bg-primary)" }}>
                    <td style={{ width: "50px", minWidth: "50px", textAlign: "center", backgroundColor: sIdx % 2 === 0 ? "var(--bg-secondary)" : "var(--bg-primary)" }}>
                      <input
                        type="checkbox"
                    ...
                    <td style={{ width: "40px", minWidth: "40px", textAlign: "center", color: "var(--text-secondary)", fontWeight: "bold", backgroundColor: sIdx % 2 === 0 ? "var(--bg-secondary)" : "var(--bg-primary)" }}>
                      {sIdx + 1}
                    </td>
                    <td 
                      ...
                      style={{ position: "sticky", left: 0, zIndex: 12, fontWeight: "600", backgroundColor: sIdx % 2 === 0 ? "var(--bg-secondary)" : "var(--bg-primary)", cursor: "pointer", boxShadow: "2px 0 5px rgba(0,0,0,0.05)" }}
                    >
                ...
                <table className="premium-table crosshair-highlight " style={{ width: "100%", minWidth: "800px" }}>
                  <thead style={{ position: "sticky", top: 0, zIndex: 20 }}>
                    <tr>
                      ...
                      <th rowSpan={hasGroups ? 2 : 1} style={{ width: "40px", minWidth: "40px", textAlign: "center", backgroundColor: "var(--bg-tertiary)", position: "sticky", left: 0, zIndex: 22 }}>
                        No
                      </th>
                      <th className="sticky-nama" rowSpan={hasGroups ? 2 : 1} style={{ position: "sticky", left: "40px", zIndex: 22, backgroundColor: "var(--bg-tertiary)", boxShadow: "4px 0 8px rgba(0,0,0,0.05)", cursor: "pointer", userSelect: "none" }} onClick={() => handleSort('nama')}>
                        Nama Siswa {sortConfig.key === 'nama' ? (sortConfig.direction === 'asc' ? '↑' : '↓') : '↕'}
                      </th>
                      <th rowSpan={hasGroups ? 2 : 1} style={{ width: "140px", minWidth: "140px", cursor: "pointer", userSelect: "none", backgroundColor: "var(--bg-tertiary)" }} onClick={() => handleSort('nisn')}>
                  ...
                  return (
                    <Fragment key={student.nisn}>
                      <tr style={{ backgroundColor: idx % 2 === 0 ? "var(--bg-secondary)" : "var(--bg-primary)" }}>
                        ...
                        <td data-label="No" style={{ width: "40px", minWidth: "40px", textAlign: "center", color: "var(--text-secondary)", fontWeight: "bold", position: "sticky", left: 0, zIndex: 5, backgroundColor: idx % 2 === 0 ? "var(--bg-secondary)" : "var(--bg-primary)" }}>
                          {idx + 1}
                        </td>
                        <td 
                          ...
                          onClick={toggleNamaExpand}
                          title="Klik untuk melihat nama lengkap"
                          style={{ fontWeight: "600", position: "sticky", left: "40px", zIndex: 5, backgroundColor: idx % 2 === 0 ? "var(--bg-secondary)" : "var(--bg-primary)", boxShadow: "4px 0 8px rgba(0,0,0,0.05)", cursor: "pointer" }}
                        >
                          <div style={{ display: "flex", alignItems: "center", gap: "8px", width: "100%", overflow: "hidden" }}>
                        ...
                        <td data-label="NISN" style={{ width: "140px", minWidth: "140px", fontFamily: "monospace", fontSize: "0.85rem", fontWeight: "600" }}>
                          {student.nisn}
                        </td>
        ...
        >
          <div style={{ display: "flex", flexDirection: "column", gap: "16px", paddingTop: "8px", paddingBottom: "16px", paddingRight: "8px", maxHeight: "70vh", overflowY: "auto" }}>
            ...
            <div className="form-group" style={{ marginTop: "8px" }}>
              <label style={{ marginBottom: "6px", display: "block", fontWeight: "600" }}>Tipe Komponen <span style={{ color: "var(--danger)" }}>*</span></label>
              <select
                className="form-input"
                value={quickAddData.isGroup ? "kelompok" : quickAddData.isPresensi ? "presensi" : "tunggal"}
                onChange={(e) => {
                  const val = e.target.value;
                  if (val === "tunggal") setQuickAddData({...quickAddData, isGroup: false, isPresensi: false});
                  else if (val === "kelompok") setQuickAddData({...quickAddData, isGroup: true, isPresensi: false});
                  else if (val === "presensi") setQuickAddData({...quickAddData, isGroup: false, isPresensi: true});
                }}
              >
                <option value="tunggal">Nilai Tunggal (Satu tugas atau ujian)</option>
                <option value="kelompok">Kelompok (Wadah untuk beberapa anak kolom)</option>
                <option value="presensi" disabled={!quickAddData.isPresensi && kelas.kolomNilai.some(c => c.isPresensi)}>
                  Integrasi Presensi {(!quickAddData.isPresensi && kelas.kolomNilai.some(c => c.isPresensi)) ? "(Sudah Digunakan)" : "(Otomatis dari kehadiran)"}
                </option>
              </select>
            </div>
              ...
              <button 
                ...
                onClick={async () => {
                  ...
                  const newKolom = {
                    ...
                    isPresensi: !quickAddData.isGroup ? quickAddData.isPresensi : false,
                    hitungMetode: quickAddData.isGroup ? quickAddData.hitungMetode : 'rata-rata',
                    subKolom: []
                  };
